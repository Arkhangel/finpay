...
перезапустился бы с начала и отправил сообщение ещё раз. Здесь это
структурно невозможно: до interrupt в узле нет ничего, кроме чтения уже
готового `payload` из state.
