...
finpay_kb — retrieval без генерации, LLM/Groq не нужны. Первый прогон — на
36 in-scope вопросах golden dataset (0.832-0.935) и 8 явно нерелевантных
("борщ"-класс, максимально далёкие темы: еда, спорт, погода) — 0.711-0.802,
чистое разделение. По просьбе честно перепроверить на менее очевидных
случаях добавили 12 "сложных" негативов — тематически СОСЕДних с FinPay
(Stripe, ЮKassa, чарджбэк в PayPal, PCI DSS вообще, эквайринг, 115-ФЗ для
банков) — они дают уже **0.794-0.859**, то есть пересекаются с in-scope
диапазоном (максимум соседнего негатива 0.859 выше минимума in-scope 0.832).

Честный вывод: чистого порога, отделяющего "похоже по теме, но не то" от
"тот самый документ", на сыром cosine НЕ существует математически — guard
надёжно ловит только очевидный шум, не смысловую релевантность. Поднять
порог до 0.85+, чтобы отсечь больше таких негативов, стоило бы ~14% (5 из
36) ложных отказов на реальные вопросы — хуже для UX, чем пропустить
тематически соседний вопрос на LLM с промпт-инструкцией "не выдумывай
факты" (второй, более надёжный слой защиты — та же логика, что и у
rerank-порога). Оставлено **0.82** — с запасом ниже всех наблюдённых
in-scope, ловит только очевидный шум. Историческая Б5.3/5.4 калибровка
(0.75) на этом корпусе оказалась бы неверной даже для easy-негативов — часть
из них (0.75-0.80) её превышала.
