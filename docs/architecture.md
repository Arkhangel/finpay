# Архитектурный паспорт: FinPay Support Assistant

## Диаграмма компонентов

```mermaid
flowchart LR
    USER["Клиент\nCLI / REST API"]

    subgraph GW["① API Gateway"]
        GW_N["nginx\nauth · rate-limit 60 RPM/IP\nTLS termination"]
    end

    subgraph SVC["② Service (FastAPI — Б3.4)"]
        SVC_A["ChatService\nBulkhead: семафор max 20 concurrent\nrequest validation"]
    end

    subgraph LLM["③ LLM Layer"]
        CACHE["Cache-Aside\nRedis · TTL 1h\nkey: sha256(model+messages+temperature)"]
        CB1["⚡ Circuit Breaker\nGroq · fail_max=5 · cooldown=60s"]
        CB2["⚡ Circuit Breaker\nOpenAI · fail_max=5 · cooldown=60s"]
        P1["Groq API\ngpt-oss-120b\n① primary · latency↓ cost↓"]
        P2["OpenAI API\ngpt-4o-mini\n② fallback · reliability↑"]
        CACHE -.->|"miss"| CB1
        CB1 -->|"ok"| P1
        CB1 -.->|"open"| CB2
        CB2 -->|"ok"| P2
    end

    subgraph DATA["④ Data Layer"]
        DB["SQLite → Postgres\ntransactions\nconversation history"]
        REDIS["Redis\nsessions · cache · rate-limit counters"]
    end

    USER --> GW_N
    GW_N --> SVC_A
    SVC_A -->|"cache lookup"| CACHE
    CACHE -.->|"hit"| SVC_A
    SVC_A --> DATA
    P1 & P2 -.->|"response"| SVC_A
    SVC_A --> USER
```

**Обозначения:**
- `⚡ Circuit Breaker` — размыкается при 5 ошибках подряд, cooldown 60 с; по одному на провайдера
- `Cache-Aside` — стоит перед LLM-слоем; при hit возвращает ответ без LLM-вызова
- `→` — основной поток запроса; `-.->` — fallback / cache hit

---

## ADR-001: Выбор паттерна взаимодействия

**Context.**
Проект — ассистент (впоследствии чат бот) техподдержки платёжного процессинга FinPay для мерчантов
и разработчиков. Сценарий: пользователь задаёт вопрос о транзакции / статусе сервиса /
политике возврата, ассистент отвечает. Ожидаемая нагрузка — 20 RPM в штатном режиме,
пик до 60 RPM; средний ответ — 200–400 токенов (2–5 с генерации).
Клиенты на текущем этапе — CLI и REST API; в М4 подключается Telegram-бот.
Бюджет — $20/мес.

**Decision.**
Выбран паттерн **Request-Response**. Клиент отправляет запрос и
получает полный ответ одним телом. Ответы поддержки — короткие и структурированные;
задержка 2–5с приемлема для консольного и REST-клиента. Function Calling требует
двух последовательных LLM-вызовов — стриминг для техподдержки не является необходимым условием,
особенно в разрезе того что мы работаем больше с мерчантами.
Даже больше бы добавил что стриминг наоборот вызовет вопросы что общение идет с ИИ, а не реальным человеком.

**Consequences**
* Преимуществом является простота и понятность взаимодействия. 
Некоторая гибкость решения: можно как ТГ бота сделать, так и чат на сайте и т.п.
* Минусом является возможные риски потери клиента при долгом формировании ответа.
Но в плане тех поддержки фин процессинга это не критично поскольку это больше взаимодействие бизнеса с бизнесом.

**Alternatives considered.**
- **Streaming (SSE)** — отвергнут на текущем этапе: усложняет двухшаговый
  Function Calling цикл; nginx требует отдельной настройки; клиентов с живым UX пока нет.
- **Queue-based (async)** — отвергнут: для интерактивной поддержки избыточен;
  добавляет worker-процессы и polling, увеличивая операционную сложность. 
  Нужен будет разве что для более точных действий типа отправить коллбэки по платежу в фоне.

---

## ADR-002: Стратегия fault tolerance

**Decision.**
Провайдеры в порядке приоритета:

| Приоритет  | Провайдер | Модель                | Обоснование                                                                                  |
|------------|-----------|-----------------------|----------------------------------------------------------------------------------------------|
| ① Primary  | Groq      | `openai/gpt-oss-120b` | Минимальная latency (TTFT <300 мс), низкая стоимость, достаточное качество для support-задач |
| ② Fallback | OpenAI    | `gpt-4o-mini`         | Высокая надёжность SLA 99.9%, широкая доступность, умеренная цена                            |

**Circuit Breaker** — библиотека `aiobreaker`, по одному экземпляру на провайдера:
`fail_max=5`, `timeout=60s`. При переходе в состояние OPEN трафик автоматически
переключается на следующий провайдер в цепочке без изменения кода клиента.

**Cache-Aside** — Redis, TTL 1 ч, ключ: `sha256(model + messages + temperature)`.
Повторные одинаковые запросы (частые вопросы о возвратах, тарифах) возвращаются
без LLM-вызова. Целевой cache hit rate: 35%.

**Consequences.**
- Сервис остаётся доступным при падении любого провайдера.
- Сложность: нужна инициализация двух CB-объектов и маршрутизация ошибок.

---

## Потенциальные точки отказа

| Слой            | Что происходит при выпадении                                      | Паттерн смягчения                                                                                  | Деградация                                                                                                                                                                               |
|-----------------|-------------------------------------------------------------------|----------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **API Gateway** | Все запросы блокируются, клиент получает connection refused / 502 | Health check + авто-рестарт nginx (systemd); резервный upstream                                    | Полная недоступность; при multi-replica — потеря одной реплики прозрачна                                                                                                                 |
| **Service**     | Запросы не обрабатываются; ошибки tool call не перехватываются    | Булкхед (семафор) защищает от перегрузки; supervisor / k8s restart                                 | При одной реплике — полная недоступность; retry на клиенте помогает при transient crash                                                                                                  |
| **LLM Layer**   | Ответы не генерируются                                            | Fallback chain (Groq → OpenAI → Anthropic) + Circuit Breaker на каждом провайдере                  | При падении одного: автопереход на следующий (<100 мс); при падении всех трёх: ответ из кеша (если hit) или template-ответ «Сервис временно недоступен, обратитесь на support@finpay.ru» |
| **Data Layer**  | Нет доступа к истории транзакций и сессиям                        | Cache-Aside (Redis) для сессий; read-only fallback на встроенный SQLite при недоступности Postgres | Инструмент `check_transaction_status` возвращает ошибку, LLM отвечает без данных транзакции; `get_payment_system_status` продолжает работать через mock                                  |

---

## Оценка нагрузки

| Параметр                    | Норма       | Пик         |
|-----------------------------|-------------|-------------|
| RPM (requests per minute)   | 20          | 60          |
| TPM (tokens per minute)     | 6 000       | 24 000      |
| Средний размер ответа       | 250 токенов | 400 токенов |
| Среднее время ответа (TTFT) | 1–2 с       | 4–5 с       |
| Бюджет $/день               | $0.30       | $1.20       |
| Целевой cache hit rate      | 35%         | 35%         |

Расчёт бюджета (пик): 60 RPM × 60 мин × 400 токенов × (1 − 0.35) = ~936 000 TPH;
при цене Groq ~$0.10/1M токенов → ~$0.09/ч → $1.08/день.

---

## LiteLLM Gateway

### Выбор: берём LiteLLM

LiteLLM Proxy предоставляет готовый OpenAI-совместимый эндпоинт с маршрутизацией
между провайдерами, retry-with-backoff, fallback chain и метриками из коробки.
Альтернатива — написать собственный роутер на базе `aiobreaker` + `tenacity` (≈200 строк).

**Решение: используем LiteLLM**. Обоснование:
- Fallback chain и Circuit Breaker декларируются в `config.yaml` без кода.
- Прозрачная смена провайдера: `client.py` не меняется — только URL и ключ.
- Встроенные метрики (Prometheus) подключаются в Б3.6 без дополнительного кода.
- При необходимости полного контроля (кастомный retry, бизнес-логика fallback)
  переход на собственный роутер возможен без изменения API клиента.

Конфиг: [`docs/litellm/config.yaml`](litellm/config.yaml)

### Локальный запуск

```bash
uv add 'litellm[proxy]'
litellm --config docs/litellm/config.yaml
# → http://localhost:4000/v1/chat/completions
```

Тест переключения (сэмулировать падение primary — неверный ключ в `GROQ_API_KEY`):

```bash
curl http://localhost:4000/v1/chat/completions \
  -H "Authorization: Bearer sk-finpay-local" \
  -H "Content-Type: application/json" \
  -d '{"model": "finpay-chat", "messages": [{"role": "user", "content": "Статус TXN-1001?"}]}'
```

При неверном `GROQ_API_KEY` LiteLLM автоматически переключится на `openai-fallback`
(видно в логах прокси: `LiteLLM: Trying model=openai-fallback`).

---

## ADR-003: Выбор embedding-модели (М5.1)

**Context.**
Дипломный RAG-ассистент индексирует базу знаний FinPay — факты о сервисе
(`app/prompts/service_facts.j2`), доки инструментов, архитектурную и
пользовательскую документацию — на русском языке. Корпус небольшой (десятки,
не миллионы документов), нагрузка на индексацию разовая/редкая (переиндексация
при обновлении доков), запросы пользователей — на русском, разговорным языком.
Запросы к эмбеддинг-модели неизбежно включают формулировки пользователей —
в финтех-поддержке это часто упоминания транзакций, сумм, магазинов, иногда
фрагментов персональных данных клиента мерчанта.

**Decision.**
Выбрана модель **`intfloat/multilingual-e5-base`** (self-hosted,
sentence-transformers, 278M параметров, 768 измерений).

Обоснование:
- **Приоритет для финтех-домена — данные не покидают периметр.** Проект уже
  выстроен вокруг того, что пользовательский текст — чувствительный: PII
  маскируется перед логированием (`app/observability/pii.py`), есть input/
  output security-слой (`app/services/security/`), модерация инцидентов
  пишется без сырого текста. Отправлять те же формулировки пользователей
  (с потенциальными фрагментами платёжных данных) третьей стороне ради
  эмбеддинга — противоречит этой модели угроз. Self-hosted модель считает
  векторы локально: текст обращения никогда не покидает инфраструктуру
  FinPay, в отличие от отправки в OpenAI Embeddings API.
- **Язык.** Хорошо ранжируется в ruMTEB retrieval — уверенное качество на
  русском, достаточное для FAQ-корпуса из коротких фактов о тарифах/статусах/
  лимитах.
- **Стоимость.** $0 за токен, нет зависимости от внешнего платного API и
  наличия ключа — с учётом того что бюджет на дополнительные cloud-ключи
  ограничен, self-hosted снимает эту зависимость полностью. По замеру
  `scripts/estimate_embedding_cost.py` на реальном корпусе проекта: ~0.175
  с/документ CPU-инференса → индексация 50 документов ≈ 8.7 с, 100 ≈ 17.5 с,
  $0 в обоих случаях.
- **Batch size.** 32 — рекомендованный диапазон для sentence-transformers на
  CPU (16–32); корпус мал, CPU-инференса достаточно, GPU не требуется.

**Alternatives considered.**
- **OpenAI `text-embedding-3-small` (cloud API).** По чистой экономике
  дешевле в абсолютных цифрах при таком маленьком корпусе (по расчёту
  `scripts/estimate_embedding_cost.py`: 50 документов ≈ $0.002), и уже
  используется в проекте как fallback LLM-провайдер (ADR-002) — интеграция
  была бы тривиальной. Отвергнут по двум причинам: (1) для финтех-поддержки
  предпочтительно не отправлять формулировки пользователей во внешний API,
  когда есть self-hosted альтернатива сравнимого качества; (2) требует
  отдельного платного ключа OpenAI (Groq, который уже используется для LLM,
  embeddings не предоставляет) — self-hosted не создаёт этой внешней
  зависимости.
- **`intfloat/multilingual-e5-small`.** Легче и быстрее (118M, 384 измерения),
  но заметно ниже качество retrieval — для небольшого FAQ-корпуса, где важно
  не путать близкие по теме факты (например, тарифы СБП и корпоративный
  тариф), выбран `-base` как более надёжный вариант.
- **`text-embedding-3-large`.** Не рассматривался всерьёз по той же причине,
  что и `-small`: cloud-вариант отвергнут на уровне класса решений, а не
  конкретной модели.

**Consequences.**
- Эмбеддинги запросов и документов считаются локально — не увеличивают
  внешнюю поверхность атаки и не меняют модель угроз проекта, в отличие от
  cloud-варианта.
- Первый запуск требует скачивания модели (~1.1GB) с HuggingFace Hub —
  разовая сетевая зависимость при разворачивании, не при каждом запросе.
- CPU-инференс медленнее API-вызова на большом объёме — для десятков
  документов и интерактивных запросов это не критично; при росте корпуса на
  порядки потребуется отдельно решать вопрос GPU или пересматривать модель.
- Смена модели (`.env`: `EMBEDDINGS__MODEL=intfloat/multilingual-e5-large`)
  автоматически инвалидирует диск-кеш — ключ кеша включает имя модели
  (`app/services/embeddings.py::_cache_key`).
- E5 — асимметричная модель: `embed_query`/`embed_documents` добавляют
  префиксы `query:`/`passage:` поверх общего `embed_texts`. Замер на
  `tests/eval/mini_benchmark.json` (`scripts/embeddings_smoke.py`) честно
  показывает небольшой, но стабильный эффект: средний gap (score relevant −
  score irrelevant) с префиксами 0.051 против 0.047 без — префиксы работают
  в ожидаемую сторону, но на этом маленьком корпусе коротких деклараций с
  "лёгкими" нерелевантными эффект не драматичный. Ожидаемо более выраженный
  эффект — на длинных документах и "трудных" негативах (близкая тема, другой
  смысл); не переоцениваем эту разницу как решающий фактор выбора модели.
- Реализация: `app/services/embeddings.py`, батчинг (`batch_size=32`),
  retry на ошибки загрузки модели (`tenacity`), диск-кеш (`diskcache`).
  Оценка стоимости индексации: `scripts/estimate_embedding_cost.py`.
  Смоук-тест кеша и E5-префиксов: `scripts/embeddings_smoke.py`.
  Mini-benchmark: `tests/eval/mini_benchmark.json`.
