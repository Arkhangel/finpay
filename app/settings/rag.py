...
class RagSettings(BaseModel):
    ...
    # без LLM — только retrieve()): 36 in-scope вопросов golden dataset дают
    # top_score 0.832-0.935 (медиана 0.887), 8 явно нерелевантных (борщ,
    # футбол, погода и т.п.) — 0.711-0.802 (медиана 0.748). Разделение чистое,
    # 0.82 — середина зазора. Старое значение 0.75 (историческая калибровка
    # Б5.4 на другом, 10-документном корпусе) на текущем корпусе НЕ работало
    # бы — часть нерелевантных вопросов (0.75-0.80) оказалась бы выше него.
    # Выборка нерелевантных вопросов маленькая (8) — грубая, но настоящая
    # калибровка, не число с потолка; расширить набор при появлении жалоб.
